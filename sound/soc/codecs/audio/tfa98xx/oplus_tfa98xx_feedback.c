				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (need_chk) {
		tfa_fb.chk_flag = CHECK_SPEAKER_MASKS + (1 << tfa_fb.pa_cnt) - 1;
	}
...
static int tfa98xx_check_status_reg(struct tfa98xx *tfa98xx)
{
	...
	pr_info("read SPK%d status regs ret=%d, reg[0x10]=0x%x, reg[0x13]=0x%x", \
			tfa98xx->tfa->dev_idx, err, reg10, reg13);
	...
	if (Tfa98xx_Error_Ok == err) {
		...
		if ((tfa98xx->pa_type == PA_TFA9874) &&
			...
			SetFdBuf(info, "TFA9874 SPK%x:reg[0x10]=0x%x,reg[0x13]=0x%x,", tfa98xx->tfa->dev_idx, reg10, reg13);
			...
			SetFdBuf(info, "TFA9873 SPK%x:reg[0x10]=0x%x,reg[0x13]=0x%x,", tfa98xx->tfa->dev_idx, reg10, reg13);
		...
		SetFdBuf(info, "%s SPK%d: failed to read regs 0x10 and 0x13, error=%d,", \
				(tfa98xx->pa_type == PA_TFA9873) ? "TFA9873" : "TFA9874", tfa98xx->tfa->dev_idx, err);
