...
static int camera_thermal_get_temp(void *data, int *temp)
{
	struct camera_thermal_info *gti = data;
	int val;
	int ret;
...
static const struct thermal_zone_of_device_ops camera_thermal_ops = {
...
MODULE_AUTHOR("ZXJ");
MODULE_DESCRIPTION("camera(flashlight+explorer) ntc driver using IIO framework with DT");
