			  ...
			  void __iomem *base, void __iomem *yuvbase)
{
	...
	for (i = 0; i < ARRAY_SIZE(fbc_r1_list); i++) {
		...
		if (fbc_r1_ctl2[i] & 0xffffff) {
			...
			writel_relaxed(fbc_r1_ctl[i] | FBC_RESET, base + REG_FBC_CTL1(FBC_R1A_BASE, i));
	...
	for (i = 0; i < ARRAY_SIZE(fbc_r2_list); i++) {
		...
		if (fbc_r2_ctl2[i] & 0xffffff) {
			...
			writel_relaxed(fbc_r2_ctl[i] | FBC_RESET, yuvbase + REG_FBC_CTL1(FBC_R2A_BASE, i));
...
#define MAX_DEBUG_SIZE (32)
	...
	if (bPrint) {
		...
		/* TODO: mtk_cam_event_error("Camsys: SW reset fail") */
#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
