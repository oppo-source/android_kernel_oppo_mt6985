...
#include "bu24721_flash_data.h"
// #define BU24721_FW_VERSION 0x1A890423
// #define BU24721_FW_VERSION 0x1A89051A
// #define BU24721_FW_VERSION 0x1A890603
// #define BU24721_FW_VERSION 0x1A890623
#define BU24721_FW_VERSION 0x1A890893
...
extern void I2C_OIS_block_write(void *register_data, int size);
...
void Prog_flash(void)
{
	...
	wdata = (SUMSEL << 31) | (AB_TYPE << 29) | (D_COPY << 27) |
		(SUM_C << 0);
	out[2] = (wdata >> 24) & 0xFF;
	out[3] = (wdata >> 16) & 0xFF;
	out[4] = (wdata >> 8) & 0xFF;
...
void WriteGyroGainToFlash(void)
{
	...
	CAM_ERR(CAM_OIS,
		"[WriteGyroGainToFlash] will write  gl_GX_OFS= 0x%x  gl_GY_OFS= 0x%x",
		X_ori, Y_ori);
	...
	Y_ori = I2C_OIS_16bit__read(Gyro_gain_y); //read 0xF07C
	CAM_ERR(CAM_OIS,
		"[WriteGyroGainToFlash] after write  gl_GX_OFS= 0x%x  gl_GY_OFS= 0x%x",
		X_ori, Y_ori);
}
...
void Gyro_gain_set(OIS_UWORD X_gain, OIS_UWORD Y_gain)
{
	...
	Y_ori = I2C_OIS_16bit__read(Gyro_gain_y); //read 0xF07C
	CAM_ERR(CAM_OIS,
		"[Gyro_gain_set] oldGyorGain  gl_GX_OFS= 0x%x  gl_GY_OFS= 0x%x",
		X_ori, Y_ori);
	CAM_ERR(CAM_OIS,
		"[Gyro_gain_set] newGyorGain  X_gain= 0x%x  Y_gain= 0x%x",
		X_gain, Y_gain);
	...
	Y_ori = I2C_OIS_16bit__read(Gyro_gain_y); //read 0xF07C
	CAM_ERR(CAM_OIS,
		"[Gyro_gain_set] after write  gl_GX_OFS= 0x%x  gl_GY_OFS= 0x%x",
		X_ori, Y_ori);
}
...
struct _FACT_ADJ Gyro_offset_cal(void)
{
	...
	Wait(70000);
	F024_Polling();
	resultGyorOffset.gl_GX_OFS =
		I2C_OIS_16bit__read(Gyro_offset_val); //0xF08A
	//Y offset
	I2C_OIS_8bit_write(Gyro_offset_lat, 0x01); //0xF088,0x01 select Y axis
	Wait(70000);
	F024_Polling();
	resultGyorOffset.gl_GY_OFS =
		I2C_OIS_16bit__read(Gyro_offset_val); //0xF08A

	CAM_ERR(CAM_OIS,
		"[Gyro_offset_cal] resultGyorOffset  gl_GX_OFS= 0x%x  gl_GY_OFS= 0x%x",
		resultGyorOffset.gl_GX_OFS, resultGyorOffset.gl_GY_OFS);

	//update to register
	I2C_OIS_8bit_write(Gyro_offset_req,
			   0x00); //0xF09C,0x00 select X axis to update
	I2C_OIS_16bit_write(
		Gyro_offset_diff,
		resultGyorOffset.gl_GX_OFS); //write the X offset value to 0xF09D
	I2C_OIS_8bit_write(Gyro_offset_req,
			   0x01); //0xF09C,0x01 select Y axis to update
	I2C_OIS_16bit_write(
		Gyro_offset_diff,
		resultGyorOffset.gl_GY_OFS); //write the Y offset value to 0xF09D

	//verify gyro offset
	I2C_OIS_8bit_write(Gyro_offset_req,
			   0x02); //0xF09C,0x02 to output X diff value
	F024_Polling();
	diff = I2C_OIS_16bit__read(
		Gyro_offset_diff); //read 0xF09D to get the X diff value
	CAM_ERR(CAM_OIS, "[Gyro_offset_cal] Gyro_offset_diff X = 0x%x", diff);
	I2C_OIS_8bit_write(Gyro_offset_req,
			   0x03); //0xF09C,0x02 to output Y diff value
	F024_Polling();
	diff = I2C_OIS_16bit__read(
		Gyro_offset_diff); //read 0xF09D to get the X diff value
	CAM_ERR(CAM_OIS, "[Gyro_offset_cal] Gyro_offset_diff Y = 0x%x", diff);

	Update_GyroCalib_to_flash(resultGyorOffset.gl_GX_OFS,
				  resultGyorOffset.gl_GY_OFS, 0, 0);

	return resultGyorOffset;
}

void Update_GyroCalib_to_flash(OIS_UWORD gyro_X_offset, OIS_UWORD gyro_Y_offset,
			       OIS_UWORD gyro_X_gain, OIS_UWORD gyro_Y_gain)
{
	...
	for (i = 0; i <= 15; i++) {
		CAM_ERR(CAM_OIS,
			"[Gyro_offset_cal] before update :buf[%d] = 0x%x", i,
			buf[i]);
	}
	...
	for (i = 0; i <= 15; i++) {
		CAM_ERR(CAM_OIS,
			"[Gyro_offset_cal] after update :buf[%d] = 0x%x", i,
			buf[i]);
	}
...
{
	struct _FACT_ADJ data;
	data = Gyro_offset_cal();
...
void Update_Gyro_offset_gain_cal_from_flash(void)
{
    ...
    en_gyrogain_update = I2C_FM_16bit__read(FLASH_ADDR_GYRO_CALIB + 3); //3DC3 check flag data
    //en_gyrogain_update = en_gyrogain_update&0x01;//judge bit[0] of 3DC3
...
void Circle_test(void)
{
	...
	Wait(3000);
	F024_Polling();
	Error = I2C_OIS_8bit__read(
		CIR_stu); //Read 0xF1A8 to get the error value
	if (Error == 4) {
		// CIR test OK
	} else {
		Error_x = I2C_OIS_16bit__read(CIR_err_x);
		Error_y = I2C_OIS_16bit__read(CIR_err_y);
		CAM_ERR(CAM_OIS, "Error_x = %04X Error_y = %04X", Error_x,
			Error_y);
...
int Rohm_bu24721_fw_download(void)
{
	...
	CAM_ERR(CAM_OIS, "Prog_ID = %08x FW_Version = %08x", Prog_ID,
		BU24721_FW_VERSION);
	...
	if (Prog_ID < BU24721_FW_VERSION) {
		if (Prog_ID == 0) {
			Boot_err_sla_change();
			CAM_ERR(CAM_OIS, "boot err change flash to %x",
				FLASH_SLVADR << 1);
		} else {
			I2C_OIS_8bit_write(Fla_strl,
					   (FLASH_SLVADR
					    << 1)); //change slave addr to 0x70
			CAM_ERR(CAM_OIS, "change flash to %x",
				FLASH_SLVADR << 1);
		}
		...
		if (!ret) {
			...
			Wait(1000);
			I2C_OIS_8bit_write(OIS_release_Standby,
					   0x00); //release standby
			ret = F024_Polling();
			CAM_ERR(CAM_OIS, "tele_ois: read 0xF004 = %d", ret);
			Prog_ID = I2C_OIS_32bit__read(
				FW_ID); //check 0xF01C program ID
			CAM_ERR(CAM_OIS, "After DL Prog_ID = %08x FW ID:%8x",
				Prog_ID, BU24721_FW_VERSION);
