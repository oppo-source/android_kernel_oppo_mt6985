...
#endif
	unsigned long ori_current[AW36515_LED_MAX];
};
 ...
 *****************************************************************************/
static int aw36515_pinctrl_init(struct aw36515_flash *flash)
{
	int ret = 0;

	/* get pinctrl */
	flash->aw36515_hwen_pinctrl = devm_pinctrl_get(flash->dev);
	if (IS_ERR(flash->aw36515_hwen_pinctrl)) {
		pr_info("Failed to get flashlight pinctrl.\n");
		ret = PTR_ERR(flash->aw36515_hwen_pinctrl);
		return ret;
	}

	/* Flashlight HWEN pin initialization */
	flash->aw36515_hwen_high = pinctrl_lookup_state(
			flash->aw36515_hwen_pinctrl,
			AW36515_PINCTRL_STATE_HWEN_HIGH);
	if (IS_ERR(flash->aw36515_hwen_high)) {
		pr_info("Failed to init (%s)\n",
			AW36515_PINCTRL_STATE_HWEN_HIGH);
		ret = PTR_ERR(flash->aw36515_hwen_high);
	}
	flash->aw36515_hwen_low = pinctrl_lookup_state(
			flash->aw36515_hwen_pinctrl,
			AW36515_PINCTRL_STATE_HWEN_LOW);
	if (IS_ERR(flash->aw36515_hwen_low)) {
		pr_info("Failed to init (%s)\n", AW36515_PINCTRL_STATE_HWEN_LOW);
		ret = PTR_ERR(flash->aw36515_hwen_low);
	}

	return ret;
}

static int aw36515_pinctrl_set(struct aw36515_flash *flash, int pin, int state)
{
	int ret = 0;
	...
	case AW36515_PINCTRL_PIN_HWEN:
		if (state == AW36515_PINCTRL_PINSTATE_LOW &&
				!IS_ERR(flash->aw36515_hwen_low))
			pinctrl_select_state(flash->aw36515_hwen_pinctrl,
					flash->aw36515_hwen_low);
		else if (state == AW36515_PINCTRL_PINSTATE_HIGH &&
				!IS_ERR(flash->aw36515_hwen_high))
			pinctrl_select_state(flash->aw36515_hwen_pinctrl,
					flash->aw36515_hwen_high);
		else
...
static int aw36515_init(struct aw36515_flash *flash)
{
	...
	aw36515_pinctrl_set(flash, AW36515_PINCTRL_PIN_HWEN, AW36515_PINCTRL_PINSTATE_HIGH);
	...
	aw36515_torch_brt_ctrl(flash, AW36515_LED0,
				flash->ori_current[AW36515_LED0]);
	aw36515_torch_brt_ctrl(flash, AW36515_LED1,
				flash->ori_current[AW36515_LED1]);
	aw36515_flash_brt_ctrl(flash, AW36515_LED0,
				flash->ori_current[AW36515_LED0]);
	aw36515_flash_brt_ctrl(flash, AW36515_LED1,
				flash->ori_current[AW36515_LED1]);
...
{
	aw36515_pinctrl_set(flash,
			AW36515_PINCTRL_PIN_HWEN, AW36515_PINCTRL_PINSTATE_LOW);
			...
			const struct i2c_device_id *devid)
{
	...
	rval = aw36515_pinctrl_init(flash);
	if (rval < 0)
		return rval;
...
MODULE_AUTHOR("XXX");
MODULE_DESCRIPTION("aw36515 LED flash v4l2 driver");
MODULE_LICENSE("GPL");
