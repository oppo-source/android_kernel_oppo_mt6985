 ...
 * Filename:
 * ---------
 *     s5khm6smipiraw_chongqing_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "s5khm6smipiraw_chongqing_Sensor.h"
#include "s5khm6smipiraw_chongqing_setting.h"
...
#define DEVICE_VERSION_CHONGQING_S5KHM6S    "s5khm6s"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5KHM6S_SENSOR_ID_CHONGQING,
...
static void sensor_init(void)
{
    ...
    hm6_burst_write_cmos_sensor(addr_data_pair_init_hm,
        sizeof(addr_data_pair_init_hm) / sizeof(kal_uint16));
/*
...
*/
    if(isDisableWriteInsensorZoomSetting == KAL_FALSE) {
        ...
        hm6_burst_write_cmos_sensor(addr_data_pair_init_hm_insensorzoom,
            sizeof(addr_data_pair_init_hm_insensorzoom) / sizeof(kal_uint16));
...
#define S5KHM6D_CHONGQING_EEPROM_I2C_ADDR 0xA0
static BYTE s5khm6s_chongqing_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_s5khm6s_chongqing_eeprom_module(kal_uint32 addr)
{
    kal_uint16 get_byte = 0;
    char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

    iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, S5KHM6D_CHONGQING_EEPROM_I2C_ADDR);

    return get_byte;
}

static void read_s5khm6s_chongqing_module_data(UINT32 sensor_id)
{
    kal_uint16 idx = 0;
    ...
    memset(s5khm6s_chongqing_common_data, 0,sizeof(s5khm6s_chongqing_common_data));
    // QR
    for(idx = 0; idx < sn_length; idx++)
    {
        s5khm6s_chongqing_common_data[8 + idx] = read_s5khm6s_chongqing_eeprom_module(sn_starAddr + idx);
    }
    // vcm
    s5khm6s_chongqing_common_data[40] = read_s5khm6s_chongqing_eeprom_module(vcmAddr);
     s5khm6s_chongqing_common_data[41] = read_s5khm6s_chongqing_eeprom_module(vcmAddr + 1);
     // lensid
    s5khm6s_chongqing_common_data[44] = read_s5khm6s_chongqing_eeprom_module(lensAddr);
    s5khm6s_chongqing_common_data[45] = read_s5khm6s_chongqing_eeprom_module(lensAddr + 1);
    // macro
    s5khm6s_chongqing_common_data[48] = read_s5khm6s_chongqing_eeprom_module(macAddr_lsb);
    s5khm6s_chongqing_common_data[49] = read_s5khm6s_chongqing_eeprom_module(macAddr_msb);
    // inf
    s5khm6s_chongqing_common_data[52] = read_s5khm6s_chongqing_eeprom_module(infAddr_lsb);
    s5khm6s_chongqing_common_data[53] = read_s5khm6s_chongqing_eeprom_module(infAddr_msb);
    // sensor id
    s5khm6s_chongqing_common_data[30] = ((sensor_id) >> 24) & 0xff;
    s5khm6s_chongqing_common_data[31] = ((sensor_id) >> 16) & 0xff;
    s5khm6s_chongqing_common_data[32] = ((sensor_id) >> 8) & 0xff;
    s5khm6s_chongqing_common_data[33] = (sensor_id) & 0xff;

    for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
        LOG_INF("cam data: %02x %02x %02x %02x\n",
               s5khm6s_chongqing_common_data[idx],
               s5khm6s_chongqing_common_data[idx + 1],
               s5khm6s_chongqing_common_data[idx + 2],
               s5khm6s_chongqing_common_data[idx + 3]);
}
            ...
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                if (deviceInfo_register_value == 0x00) {
                    register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION_CHONGQING_S5KHM6S, module_id);
                    read_s5khm6s_chongqing_module_data(*sensor_id);
...
// #ifdef OPLUS_FEATURE_CAMERA_COMMON
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, s5khm6s_chongqing_common_data,
