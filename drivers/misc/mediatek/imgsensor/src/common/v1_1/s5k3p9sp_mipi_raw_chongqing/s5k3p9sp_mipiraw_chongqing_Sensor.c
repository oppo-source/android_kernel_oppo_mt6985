...
#include "s5k3p9sp_mipiraw_chongqing_Sensor.h"
...
#define DEVICE_VERSION_S5k3P9SP_CHONGQING     "s5k3p9sp_chongqing"
...
#define PFX "S5K3P9SP_chongqing_camera_sensor"
#define FOUR_CELL_XTC_CAL_SIZE 2048
#define FOUR_CELL_XTC_CAL_ADDR 0x0E00
static char FOUR_CELL_XTC_CAL[FOUR_CELL_XTC_CAL_SIZE + 2];
#define FOUR_CELL_REMOSAIC_CAL_SIZE (FOUR_CELL_XTC_CAL_SIZE)
...
static struct imgsensor_info_struct imgsensor_info = {
		.sensor_id = S5K3P9SP_SENSOR_ID_CHONGQING,
		#ifdef OPLUS_FEATURE_CAMERA_COMMON
		.module_id = 0x02,	//0x01 Sunny,0x05 QTEK
		#endif
...
unsigned int chongqing_read_4cell_data(char *data)
{
	if (data != NULL) {
...
#ifdef USE_TNP_BURST
const u16 chongqing_uTnpArrayInit[] = {
	0x126F,
	0x0000,
	0x0000,
	0x4906,
...
static void burst_read_to_check(void)
{
	...
	for(i = 0; i < (u16)sizeof(chongqing_uTnpArrayInit); i++)
	{
...
static void sensor_init(void)
{
	...
	iWriteRegI2C((u8*)chongqing_uTnpArrayInit , (u16)sizeof(chongqing_uTnpArrayInit), imgsensor.i2c_write_id);
...
//#ifdef OPLUS_FEATURE_CAMERA_COMMON
#define S5K3P9SP_CHONGQING_EEPROM_I2C_ADDR 0xA8
static BYTE s5k3p9sp_chongqing_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_s5k3p9sp_chongqing_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, S5K3P9SP_CHONGQING_EEPROM_I2C_ADDR);

	return get_byte;
}
static void read_s5k3p9sp_chongqing_module_data(UINT32 sensor_id)
{
// s5k3p9sp_chongqing_common_data
// length   : 64
// QR       : [8 ~ 26]
// vcm      : [40 ~ 41]
// lensid   : [44 ~ 45]
// sensorid : [30 ~ 33]
	...
	memset(s5k3p9sp_chongqing_common_data, 0,sizeof(s5k3p9sp_chongqing_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		s5k3p9sp_chongqing_common_data[8 + idx] = read_s5k3p9sp_chongqing_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	s5k3p9sp_chongqing_common_data[40] = read_s5k3p9sp_chongqing_eeprom_module(vcmAddr);
	s5k3p9sp_chongqing_common_data[41] = read_s5k3p9sp_chongqing_eeprom_module(vcmAddr + 1);
	//lensid
	s5k3p9sp_chongqing_common_data[44] = read_s5k3p9sp_chongqing_eeprom_module(lensAddr);
	s5k3p9sp_chongqing_common_data[45] = read_s5k3p9sp_chongqing_eeprom_module(lensAddr + 1);
    //sensor id
    s5k3p9sp_chongqing_common_data[30] = ((sensor_id) >> 24) & 0xff;
	s5k3p9sp_chongqing_common_data[31] = ((sensor_id) >> 16) & 0xff;
	s5k3p9sp_chongqing_common_data[32] = ((sensor_id) >> 8) & 0xff;
	s5k3p9sp_chongqing_common_data[33] = (sensor_id) & 0xff;

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		LOG_INF("cam data: %02x %02x %02x %02x\n",
		       s5k3p9sp_chongqing_common_data[idx],
		       s5k3p9sp_chongqing_common_data[idx + 1],
		       s5k3p9sp_chongqing_common_data[idx + 2],
		       s5k3p9sp_chongqing_common_data[idx + 3]);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = (((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001))+SENSOR_ID_OFFSET_CHONGQING);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				printk("s5k3p9sp_module_id=%d\n",imgsensor_info.module_id);
				imgsensor_info.module_id = 0x05;  // 00X05 QTECH
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_S5k3P9SP_CHONGQING, imgsensor_info.module_id);
					read_s5k3p9sp_chongqing_module_data(*sensor_id);
...
//	#endif
	...
	*feature_para_len = 4;
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
		{
			...
			read_4cell_from_eeprom(FOUR_CELL_XTC_CAL,
			FOUR_CELL_XTC_CAL_ADDR, FOUR_CELL_XTC_CAL_SIZE);
		memcpy(data, FOUR_CELL_XTC_CAL, FOUR_CELL_XTC_CAL_SIZE+2);
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, s5k3p9sp_chongqing_common_data,
