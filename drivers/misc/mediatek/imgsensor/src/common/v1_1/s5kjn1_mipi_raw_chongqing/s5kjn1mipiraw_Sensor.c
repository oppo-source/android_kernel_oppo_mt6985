...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1_SENSOR_ID_CHONGQING,
...
// //#ifdef OPLUS_FEATURE_CAMERA_COMMON
static BYTE s5kjn1_chongqing_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_s5kjn1_chongqing_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
...
static void read_s5kjn1_chongqing_module_data(UINT32 sensor_id)
{
// s5kjn1_chongqing_common_data
// length   : 64
// QR       : [8 ~ 26]
// vcm      : [40 ~ 41]
// lensid   : [44 ~ 45]
...
// sensorid : [30 ~ 33]
	...
	memset(s5kjn1_chongqing_common_data, 0,sizeof(s5kjn1_chongqing_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		s5kjn1_chongqing_common_data[8 + idx] = read_s5kjn1_chongqing_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	s5kjn1_chongqing_common_data[40] = read_s5kjn1_chongqing_eeprom_module(vcmAddr);
	s5kjn1_chongqing_common_data[41] = read_s5kjn1_chongqing_eeprom_module(vcmAddr + 1);
	//lensid
	s5kjn1_chongqing_common_data[44] = read_s5kjn1_chongqing_eeprom_module(lensAddr);
	s5kjn1_chongqing_common_data[45] = read_s5kjn1_chongqing_eeprom_module(lensAddr + 1);
	//macro
	s5kjn1_chongqing_common_data[48] = read_s5kjn1_chongqing_eeprom_module(macAddr_lsb);
	s5kjn1_chongqing_common_data[49] = read_s5kjn1_chongqing_eeprom_module(macAddr_msb);
	//inf
	s5kjn1_chongqing_common_data[52] = read_s5kjn1_chongqing_eeprom_module(infAddr_lsb);
	s5kjn1_chongqing_common_data[53] = read_s5kjn1_chongqing_eeprom_module(infAddr_msb);
	//sensor id
	s5kjn1_chongqing_common_data[30] = ((sensor_id) >> 24) & 0xff;
	s5kjn1_chongqing_common_data[31] = ((sensor_id) >> 16) & 0xff;
	s5kjn1_chongqing_common_data[32] = ((sensor_id) >> 8) & 0xff;
	s5kjn1_chongqing_common_data[33] = (sensor_id) & 0xff;

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		LOG_INF("cam data: %02x %02x %02x %02x\n",
		       s5kjn1_chongqing_common_data[idx],
		       s5kjn1_chongqing_common_data[idx + 1],
		       s5kjn1_chongqing_common_data[idx + 2],
		       s5kjn1_chongqing_common_data[idx + 3]);
}
...
//#ifdef OPLUS_FEATURE_CAMERA_COMMON
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, s5kjn1_chongqing_common_data,
