...
#include "ov64bmipiraw_chongqing_Sensor.h"
#include "ov64bmipiraw_chongqing_setting.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV64B_SENSOR_ID_CHONGQING,
	.module_id = 0x07,
...
#define OV64B_CHONGQING_EEPROM_I2C_ADDR 0xA0
static BYTE ov64b_chongqing_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_ov64b_chongqing_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, OV64B_CHONGQING_EEPROM_I2C_ADDR);

	return get_byte;
}

static void read_ov64b_chongqing_module_data()
{
	kal_uint16 idx = 0;
	...
	memset(ov64b_chongqing_common_data, 0,sizeof(ov64b_chongqing_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		ov64b_chongqing_common_data[8 + idx] = read_ov64b_chongqing_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	ov64b_chongqing_common_data[40] = read_ov64b_chongqing_eeprom_module(vcmAddr);
	ov64b_chongqing_common_data[41] = read_ov64b_chongqing_eeprom_module(vcmAddr + 1);
	//lensid
	ov64b_chongqing_common_data[44] = read_ov64b_chongqing_eeprom_module(lensAddr);
	ov64b_chongqing_common_data[45] = read_ov64b_chongqing_eeprom_module(lensAddr + 1);
	//macro
	ov64b_chongqing_common_data[48] = read_ov64b_chongqing_eeprom_module(macAddr_lsb);
	ov64b_chongqing_common_data[49] = read_ov64b_chongqing_eeprom_module(macAddr_msb);
	//inf
	ov64b_chongqing_common_data[52] = read_ov64b_chongqing_eeprom_module(infAddr_lsb);
	ov64b_chongqing_common_data[53] = read_ov64b_chongqing_eeprom_module(infAddr_msb);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		LOG_INF("cam data: %02x %02x %02x %02x\n",
		       ov64b_chongqing_common_data[idx],
		       ov64b_chongqing_common_data[idx + 1],
		       ov64b_chongqing_common_data[idx + 2],
		       ov64b_chongqing_common_data[idx + 3]);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = return_sensor_id()+31;
					printk("ov64b i2c write id: 0x%x, sensor id: 0x%x\n",imgsensor.i2c_write_id, *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION, imgsensor_info.module_id);
					read_ov64b_chongqing_module_data();
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		printk("ov64b get_imgsensor_id: 0x%x fail\n", *sensor_id);
            ...
            UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    *(feature_data + 1) = imgsensor_info.min_shutter;
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, ov64b_chongqing_common_data,
