...
#include "ov02b1bmipimono_chongqing_Sensor.h"
...
#define DEVICE_VERSION  "ov02b1b"
//#endif

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B1B_SENSOR_ID_CHONGQING,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7]={
	...
	{  1600, 1200,  0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200,  0, 0, 1600, 1200}  /* custom2 24fps */
};
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	ret =iReadRegI2C(pu_send_cmd, 1, (u8*)&get_byte, 1, imgsensor.i2c_write_id);
...
//#ifdef OPLUS_FEATURE_CAMERA_COMMON
#define OV02B1B_MONO_CHONGQING_EEPROM_I2C_ADDR 0xA2
static BYTE ov02b1b_mono_chongqing_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static void read_ov02b1b_mono_chongqing_module_data(UINT32 sensor_id)
{
// ov02b1b_mono_chongqing_common_data
// length   : 64
// sensorid : [30 ~ 33]

	kal_uint16 idx = 0;
	memset(ov02b1b_mono_chongqing_common_data, 0,sizeof(ov02b1b_mono_chongqing_common_data));
	//sensor id
	ov02b1b_mono_chongqing_common_data[30] = ((sensor_id) >> 24) & 0xff;
	ov02b1b_mono_chongqing_common_data[31] = ((sensor_id) >> 16) & 0xff;
	ov02b1b_mono_chongqing_common_data[32] = ((sensor_id) >> 8) & 0xff;
	ov02b1b_mono_chongqing_common_data[33] = (sensor_id) & 0xff;

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		LOG_INF("cam data: %02x %02x %02x %02x\n",
		       ov02b1b_mono_chongqing_common_data[idx],
		       ov02b1b_mono_chongqing_common_data[idx + 1],
		       ov02b1b_mono_chongqing_common_data[idx + 2],
		       ov02b1b_mono_chongqing_common_data[idx + 3]);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("ov02b1b get_imgsensor_id----1  0x%x\n",imgsensor_info.sensor_id);
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			LOG_INF("ov02b1b get_imgsensor_id----2  0x%x\n",*sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
			LOG_INF("find sensor: 0x%x, sensor id: 0x%x\n",
			imgsensor.i2c_write_id, *sensor_id);
			if (deviceInfo_register_value == 0) {
				register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION, imgsensor_info.module_id);
				deviceInfo_register_value = 1;
			}
			if (0x7A == imgsensor.i2c_write_id) { // mono
				#if OV02B1B_EEPROM_ENABLE
...
//#ifdef OPLUS_FEATURE_CAMERA_COMMON
			read_ov02b1b_mono_chongqing_module_data(*sensor_id);
//#endif  /* OPLUS_FEATURE_CAMERA_COMMON */
...
static kal_uint32 custom2(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
    MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_info("[ov02b1b] custom2 mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
//#ifdef OPLUS_FEATURE_CAMERA_COMMON
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, ov02b1b_mono_chongqing_common_data,
