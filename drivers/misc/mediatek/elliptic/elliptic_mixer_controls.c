...
static uint32_t ultrasound_enable_debug_log;

int elliptic_ultrasound_enable_debug_log_get(struct snd_kcontrol *kcontrol,
                      struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.integer.value[0] = ultrasound_enable_debug_log;
    return 0;
}

int elliptic_ultrasound_enable_debug_log_set(struct snd_kcontrol *kcontrol,
                      struct snd_ctl_elem_value *ucontrol)
{
    int32_t msg[4] = {0, 0, 0, 0};

    ultrasound_enable_debug_log = ucontrol->value.integer.value[0];

    msg[0] = ultrasound_enable_debug_log ? 1 : 0;

    return elliptic_data_write(
        ELLIPTIC_ENABLE_DEBUG_LOG,
...
static const struct snd_kcontrol_new ultrasound_filter_mixer_controls[] = {
    ...
    SOC_ENUM_EXT("Ultrasound Enable Debug Log",
    elliptic_enum[0],
    elliptic_ultrasound_enable_debug_log_get,
    elliptic_ultrasound_enable_debug_log_set),
...
unsigned int elliptic_add_platform_controls(void *component)
{
	...
	if (component != NULL) {
#ifdef LINUX_5X  /*Using the component instead of platform from linux5.x. For old platform, changes to 0 HERE*/
