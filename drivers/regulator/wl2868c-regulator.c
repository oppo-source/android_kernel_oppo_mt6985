			     ...
			     const struct i2c_device_id *id)
{
	...
	int access_time = 3;
	unsigned int version = PCB_UNKNOWN;
	...
	if (version < PCB_VERSION_PVT1) {
		pr_info("PCB VERSION[%u] < PVT1, close wl2868c-pmic OCP\n", version);
		ret = regmap_write(pdata->regmap, 0x11, 0x00);
		if (ret) {
			dev_err(&i2c->dev, "Failed to close tele sensor OCP, ret = %d\n", ret);
		}
		ret = regmap_write(pdata->regmap, 0x13, 0x00);
		if (ret) {
			dev_err(&i2c->dev, "Failed to close mian sensor OCP, ret = %d\n", ret);
		}
	} else{
		pr_info("PCB VERSION[%u] >= PVT1, not close wl2868c-pmic OCP\n", version);
	}
