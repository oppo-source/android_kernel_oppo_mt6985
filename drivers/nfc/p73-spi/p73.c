...
struct p61_dev {
    ...
    /* system lock */
    spinlock_t slock;
    struct wakeup_source *ese_wakelock;
    //struct mutex ese_lock;
#endif
};
...
static void p61_spi_clk_enable(struct p61_dev *p61_dev)
{
    ...
    spin_lock_irqsave(&p61_dev->slock, flags);
    //if (!p61_dev->ese_wakelock->active)
    //        __pm_stay_awake(p61_dev->ese_wakelock);
    if (!p61_dev->ese_wakelock->active)
            __pm_wakeup_event(p61_dev->ese_wakelock, 5000);
    spin_unlock_irqrestore(&p61_dev->slock, flags);
    pr_err("%s, __pm_stay_awake PM awake lock \n", __func__);
#endif
...
static void p61_spi_clk_disable(struct p61_dev *p61_dev)
{
    ...
    unsigned long flags = 0;
    spin_lock_irqsave(&p61_dev->slock, flags);
    __pm_relax(p61_dev->ese_wakelock);
    spin_unlock_irqrestore(&p61_dev->slock, flags);
    pr_err("%s, __pm_relax PM awake lock \n", __func__);
#endif
...
#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
    p61_dev->ese_wakelock =
                wakeup_source_register(NULL, "ese wakelock");
    if (!p61_dev->ese_wakelock) {
                P61_ERR_MSG("wakeup source init failed.\n");
                goto err_exit0;
    }
    spin_lock_init(&p61_dev->slock);
#endif
