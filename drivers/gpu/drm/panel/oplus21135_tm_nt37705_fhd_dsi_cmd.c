	...
	void *minfps, struct drm_display_mode *m)
{
	...
	ADFRFUNC("mode_id:%u,refresh_rate:%u,minfps_flag:%u,extern_frame:%u\n",
	...
	if (!min_fps->minfps_flag) {
		...
		if (get_pwm_status()) {
			if (vrefresh_rate != 90) {
				...
				ADFRFUNC("auto_off_minfps_cmd_hpwm_120hz,ext_frame:%u\n", ext_frame);
			...
			if (vrefresh_rate != 90) {
				...
				ADFRFUNC("auto_off_minfps_cmd_lpwm_120hz,ext_frame:%u\n", ext_frame);
				...
				ADFRFUNC("auto_off_minfps_cmd_lpwm_90hz,ext_frame:%u\n", ext_frame);
...
#ifdef OPLUS_FEATURE_DISPLAY_TEMP_COMPENSATION
	if (oplus_temp_compensation_register_ntc_channel(dev)) {
